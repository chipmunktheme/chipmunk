<div class="c-ratings">
    ...
    <div class="c-ratings__form <?php echo esc_attr(!empty($ratings['rating']) ? 'is-active' : ''); ?>">
        <?php for ($i = $max_rating; $i >= 1; $i--) : ?>
            <button class="c-ratings__button <?php echo esc_attr($i == round($ratings['average']) ? 'is-selected' : ''); ?> <?php echo esc_attr(!empty($ratings['rating']) && $i == $ratings['rating']['rating'] ? 'is-active' : ''); ?>" title="<?php printf(esc_attr(_n('%d star', '%d stars', $i, 'chipmunk')), $i); ?>" data-rating data-action="submit_rating" data-action-rating="<?php echo esc_attr($i); ?>" data-action-post-id="<?php echo esc_attr(get_the_ID()); ?>">
