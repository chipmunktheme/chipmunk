...
<?php if ( $query && $query->max_num_pages > 1 && ( ! isset( $query->query['orderby'] ) || $query->query['orderby'] != 'rand' ) ) : ?>
	<nav class="c-pagination l-component l-component--md">
		<?php if ( Chipmunk\Helpers::getOption( 'pagination_type' ) == 'load_more' || Chipmunk\Helpers::getOption( 'pagination_type' ) == 'infinite' ) : ?>
			<button class="c-pagination__button c-button c-button--primary-outline u-loader" data-action="load_posts" data-query-vars="<?php echo esc_attr( json_encode( $query->query ) ); ?>" data-page="<?php echo $paged + 1; ?>"<?php echo Chipmunk\Helpers::getOption( 'pagination_type' ) == 'infinite' ? ' data-view-trigger="click"' : ''; ?>>
