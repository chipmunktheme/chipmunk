...
<<?php echo get_post_status() == 'publish' ? 'a href="' . get_the_permalink() . '"' : 'article'; ?> class="<?php echo esc_attr( $tile_classes[ chipmunk_theme_option( 'display_resource_as' ) ] ); ?><?php echo $is_column ? ' column column--md-3 column--lg-4' : ''; ?>">
	...
	<div class="tile__content<?php echo ( chipmunk_theme_option( 'display_resource_as' ) == 'tile' ? ( chipmunk_theme_option( 'disable_resource_thumbs' ) || ! has_post_thumbnail() ? ' tile__content--primary' : ' tile__content--dimmed' ) : '' ); ?>">
		<div class="tile__info">
			<div class="tile__head">
				...
				<?php if ( ! chipmunk_theme_option( 'disable_resource_website_button' ) and ! empty( $resource_website ) ) : ?>
					<script>
			...
			<?php if ( ! chipmunk_theme_option( 'disable_resource_desc' ) and ! empty( $content ) ) : ?>
				<p class="tile__copy"><?php echo esc_html( chipmunk_truncate_string( $content, apply_filters( 'chipmunk_excerpt_length', ( chipmunk_theme_option( 'display_resource_as' ) == 'card_blank' ? 80 : 60 ) ) ) ); ?><span>&nbsp;<?php chipmunk_get_template( 'partials/icon', array( 'icon' => 'arrow-right' ) ); ?></span></p>
			<?php endif; ?>
