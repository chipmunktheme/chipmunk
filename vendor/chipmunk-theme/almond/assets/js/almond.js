...
var Almond = (function($){
    ...
    function window_loaded(){
    	...
    	var
    	body 		= $('.almond__body'),
    	body_loading 	= $('.almond__body--loading'),
    	body_exiting 	= $('.almond__body--exiting'),
    	drawer_trigger 	= $('#almond__drawer-trigger'),
    	drawer_opening 	= 'almond__drawer--opening';
    	drawer_opened 	= 'almond__drawer--open';
    	...
    	$('.almond__button--proceed:not(.almond__button--closer)').click(function (e) {
		    e.preventDefault();
        ...
        $(".almond__button--closer").on('click', function(e){
        ...
        $(".button-next").on( "click", function(e) {
            e.preventDefault();
            var loading_button = almond_loading_button(this);
            if ( ! loading_button ) {
                return false;
            }
            ...
            if( data_callback && typeof callbacks[data_callback] !== "undefined"){
				...
				$( document ).on( 'change', '.js-almond-demo-import-select', function() {
					var selectedIndex  = $( this ).val();

					$( '.js-almond-select-spinner' ).show();

					$.post( almond_params.ajaxurl, {
						action: 'almond_update_selected_import_data_info',
						wpnonce: almond_params.wpnonce,
						selected_index: selectedIndex,
					}, function( response ) {
						if ( response.success ) {
							$( '.js-almond-drawer-import-content' ).html( response.data );
						}
						else {
							alert( almond_params.texts.something_went_wrong );
						}

						$( '.js-almond-select-spinner' ).hide();
					} )
						.fail( function() {
							$( '.js-almond-select-spinner' ).hide();
							alert( almond_params.texts.something_went_wrong )
    ...
    function ChildTheme() {
        ...
        function do_ajax() {
            jQuery.post(almond_params.ajaxurl, {
                action: "almond_child_theme",
                wpnonce: almond_params.wpnonce,
        ...
        return {
            init: function(btn) {
                complete = function() {
                	...
                	setTimeout(function(){
				$(".almond__body").addClass('js--finished');
			},1500);

                	body.removeClass( drawer_opened );

                	setTimeout(function(){
				$('.almond__body').addClass('exiting');
			},3500);
...
function ActivateLicense() {
    	var body 		= $( '.almond__body' );
    	var wrapper 		= $( '.almond__content--license-key' );
        ...
        function ajax_callback(r) {
            ...
            if (typeof r.success !== "undefined" && r.success) {
              notice.siblings( '.error-message' ).remove();
                ...
                $( '.js-almond-license-activate-button' ).removeClass( 'almond__button--loading' ).data( 'done-loading', 'no' );
                notice.siblings( '.error-message' ).remove();
        ...
        function do_ajax() {
            ...
            jQuery.post(almond_params.ajaxurl, {
              action: "almond_activate_license",
              wpnonce: almond_params.wpnonce,
...
function PluginManager(){
        ...
        function find_next(){
            ...
            var $li = $(".almond__drawer--install-plugins li");
            $li.each(function(){
                var $item = $(this);
        ...
        return {
            init: function(btn){
                $(".almond__drawer--install-plugins").addClass("installing");
                $(".almond__drawer--install-plugins").find("input").prop("disabled", true);
                complete = function(){
    ...
    function ContentManager(){
                ...
                if(typeof response.url !== "undefined"){
                    ...
                    if(response.hash === current_item_hash){
                        ...
                        if ( typeof response.selected_index === "undefined" ) {
                            response.selected_index = $( '.js-almond-demo-import-select' ).val() || 0;
                        }
            ...
            if(current_item){
                ...
                if($check.is(":checked")) {
                    jQuery.post(almond_params.ajaxurl, {
                        ...
                        selected_index: $( '.js-almond-demo-import-select' ).val() || 0
                    }, ajax_callback).fail(ajax_callback);
                }else{
                    $current_node.addClass("skipping");
                    setTimeout(find_next,300);
        ...
        function find_next(){
            ...
            var $items = $(".almond__drawer--import-content__list-item");
            var $enabled_items = $(".almond__drawer--import-content__list-item input:checked");
            $items.each(function(){
        ...
        function init_content_import_progress_bar() {
            if( ! $(".almond__drawer--import-content__list-item .checkbox-content").is( ':checked' ) ) {
                return false;
            }

            jQuery.post(almond_params.ajaxurl, {
                action: "almond_get_total_content_import_items",
                wpnonce: almond_params.wpnonce,
                selected_index: $( '.js-almond-demo-import-select' ).val() || 0
            }, function( response ) {
        ...
        function update_progress_bar() {
            $('.js-almond-progress-bar').css( 'width', (current_content_import_items/total_content_import_items) * 100 + '%' );

            var $percentage = valBetween( ((current_content_import_items/total_content_import_items) * 100) , 0, 99);

            $('.js-almond-progress-bar-percentage').html( Math.round( $percentage ) + '%' );
        ...
        return {
            init: function(btn){
                $(".almond__drawer--import-content").addClass("installing");
                $(".almond__drawer--import-content").find("input").prop("disabled", true);
                complete = function(){

			$.post(almond_params.ajaxurl, {
				action: "almond_import_finished",
				wpnonce: almond_params.wpnonce,
				selected_index: $( '.js-almond-demo-import-select' ).val() || 0
			});

			setTimeout(function(){
				$('.js-almond-progress-bar-percentage').html( '100%' );
			},100);
    ...
    function almond_loading_button( btn ){
        ...
        $button.addClass("almond__button--loading");
