...
*/
! ( function ( a ) {
	typeof a.fn.each2 === 'undefined' &&
		a.extend( a.fn, {
			each2 ( b ) {
				for (
					let c = a( [ 0 ] ), d = -1, e = this.length;
					++d < e &&
	...
	( function ( a, b ) {
		...
		function n( b ) {
			const c = a( document.createTextNode( '' ) );
		...
		function p( a, b ) {
			for ( let c = 0, d = b.length; d > c; c += 1 )
				if ( r( a, b[ c ] ) ) return c;
			return -1;
		}
		function q() {
		...
		function y( a ) {
			a[ 0 ] !== document.activeElement &&
				window.setTimeout( function () {
					let d;
						const b = a[ 0 ];
						const c = a.val().length;
					a.focus();
					const e = b.offsetWidth > 0 || b.offsetHeight > 0;
					e &&
		...
		function z( b ) {
			...
			if ( 'selectionStart' in b )
				...
				b.focus();
				const e = document.selection.createRange();
		...
		function E( a, b, c, d ) {
			const e = o( a.toUpperCase() ).indexOf( o( b.toUpperCase() ) );
				const f = b.length;
			return e < 0
				? ( c.push( d( a ) ), void 0 )
						...
						( d = d
							...
							( f =
								typeof f === 'function'
									...
									: f ),
							e && typeof e.abort === 'function' && e.abort(),
							c.params &&
		...
		if ( window.Select2 === b ) {
			let c;
				...
				var k = {
					...
					isFunctionKey ( a ) {
						return (
							( a = a.which ? a.which : a ), a >= 112 && a <= 123
						);
					},
				};
				var l = "<div class='select2-measure-scrollbar'></div>";
				var m = {
					'\u24b6': 'A',
					'\uff21': 'A',
					'\xc0': 'A',
			...
			( i = a( document ) ),
				...
				( c = O( Object, {
					...
					init ( c ) {
						...
						( this.opts = c = this.prepareOpts( c ) ),
							...
							c.maximumInputLength !== null &&
								this.search.attr(
									'maxlength',
									c.maximumInputLength
								);
						let h = c.element.prop( 'disabled' );
						h === b && ( h = ! 1 ), this.enable( ! h );
						let i = c.element.prop( 'readonly' );
						i === b && ( i = ! 1 ),
							this.readonly( i ),
							( j = j || q() ),
					...
					destroy () {
						const a = this.opts.element;
							const c = a.data( 'select2' );
							const d = this;
						this.close(),
							a.length &&
								a[ 0 ].detachEvent &&
					...
					prepareOpts ( c ) {
						...
						if (
							( ( d = c.element ),
							d.get( 0 ).tagName.toLowerCase() === 'select' &&
							...
							( c = a.extend(
								{},
								{
									populateResults ( d, e, g ) {
										let h;
											const j = this.opts.id;
											const k = this.liveRegion;
										( h = function ( d, e, l ) {
											let m; let n; let o; let p; let q; let r; let s; let t; let u; let v;
						...
						if ( c.createSearchChoicePosition === 'top' )
							c.createSearchChoicePosition = function ( a, b ) {
								a.unshift( b );
							};
						else if ( c.createSearchChoicePosition === 'bottom' )
							c.createSearchChoicePosition = function ( a, b ) {
								a.push( b );
							};
						else if (
							typeof c.createSearchChoicePosition !== 'function'
						)
					...
					monitorSource () {
						...
						c.on(
							...
							( this._sync = this.bind( function () {
								let a = c.prop( 'disabled' );
								a === b && ( a = ! 1 ), this.enable( ! a );
								let d = c.prop( 'readonly' );
								d === b && ( d = ! 1 ),
									this.readonly( d ),
									this.container &&
										( D(
					...
					triggerSelect ( b ) {
						const c = a.Event( 'select2-selecting', {
							val: this.id( b ),
							object: b,
							choice: b,
						} );
					...
					enableInterface () {
						const a = this._enabled && ! this._readonly;
							const b = ! a;
					...
					positionDropdown () {
						let v;
							...
							let d = c.offset();
							let e = c.outerHeight( ! 1 );
							let f = c.outerWidth( ! 1 );
							let g = b.outerHeight( ! 1 );
							const h = a( window );
							const i = h.width();
							const k = h.height();
							let l = h.scrollLeft() + i;
							let m = h.scrollTop() + k;
							let n = d.top + e;
							let o = d.left;
							const p = m >= n + g;
							const q = d.top - g >= h.scrollTop();
							let r = b.outerWidth( ! 1 );
							const s = function () {
								return l >= o + r;
							};
							const t = function () {
							...
							const u = b.hasClass( 'select2-drop-above' );
						u
							? ( ( w = ! 0 ),
							  ! q && p && ( ( x = ! 0 ), ( w = ! 1 ) ) )
							: ( ( w = ! 1 ),
					...
					opening () {
						let f;
							const b = this.containerEventName;
							const c = `scroll.${  b}`;
							const d = `resize.${  b}`;
							const e = `orientationchange.${  b}`;
						this.container
					...
					ensureHighlightVisible () {
						...
						if ( ( ( d = this.highlight() ), ! ( d < 0 ) ) ) {
							if ( d == 0 ) return b.scrollTop( 0 ), void 0;
					...
					moveHighlight ( b ) {
						for (
							let c = this.findHighlightableChoices(),
								d = this.highlight();
							d > -1 && d < c.length;
					...
					highlight ( b ) {
						let d;
							let e;
							const c = this.findHighlightableChoices();
					...
					highlightUnderEvent ( b ) {
						const c = a( b.target ).closest(
							'.select2-result-selectable'
						);
						if (
							c.length > 0 &&
							...
							const d = this.findHighlightableChoices();
							this.highlight( d.index( c ) );
						} else c.length == 0 && this.removeHighlight();
					},
					loadMoreIfNeeded () {
						let c;
							const a = this.results;
							const b = a.find( 'li.select2-more-results' );
							const d = this.resultsPage + 1;
							const e = this;
							const f = this.search.val();
					...
					updateResults ( c ) {
						...
						let g;
							...
							var h = this;
							const j = d.val();
							const k = a.data( this.container, 'select2-last-term' );
						if (
							( c === ! 0 || ! k || ! r( j, k ) ) &&
							...
							l = ++this.queryCount;
							const o = this.getMaximumSelectionSize();
							if (
								o >= 1 &&
								( ( g = this.data() ),
								a.isArray( g ) &&
					...
					selectHighlighted ( a ) {
						...
						const b = this.highlight();
							const c = this.results.find( '.select2-highlighted' );
							const d = c
					...
					getPlaceholderOption () {
						if ( this.select ) {
							const c = this.select.children( 'option' ).first();
					...
					initContainerWidth () {
						function c() {
							let c; let d; let e; let f; let g; let h;
							if ( this.opts.width === 'off' ) return null;
							if ( this.opts.width === 'element' )
								return this.opts.element.outerWidth( ! 1 ) === 0
									? 'auto'
								...
								if (
									...
									for (
										...
										if (
											...
											return e[ 1 ];
								return this.opts.width === 'resolve'
						...
						const d = c.call( this );
						d !== null && this.container.css( 'width', d );
					},
				} ) ),
				( d = O( c, {
					createContainer () {
					...
					initContainer () {
						...
						const h = this.opts.element.attr( 'title' );
					...
					clear ( b ) {
						const c = this.selection.data( 'select2-data' );
						if ( c ) {
							const d = a.Event( 'select2-clearing' );
							if (
								( this.opts.element.trigger( d ),
								d.isDefaultPrevented() )
							)
								return;
							const e = this.getPlaceholderOption();
					...
					prepareOpts () {
						const b = this.parent.prepareOpts.apply(
								this,
								arguments
							);
							const c = this;
					...
					postprocessResults ( a, b, c ) {
						...
						if (
							...
							const g = this.opts.minimumResultsForSearch;
					...
					updateSelection ( a ) {
						let d;
							let e;
							const c = this.selection.find( '.select2-chosen' );
				...
				( e = O( c, {
					...
					clearSearch () {
						const a = this.getPlaceholder();
							const c = this.getMaxSearchWidth();
						a !== b &&
					...
					unselect ( b ) {
						...
						if ( ( d = b.data( 'select2-data' ) ) ) {
							const f = a.Event( 'select2-removing' );
							if (
								( ( f.val = this.id( d ) ),
								( f.choice = d ),
					...
					postprocessResults ( a, b, c ) {
						const d = this.getVal();
							const e = this.results.find( '.select2-result' );
							const f = this.results.find(
								'.select2-result-with-children'
							);
							const g = this;
						e.each2( function ( a, b ) {
							const c = g.id( b.data( 'select2-data' ) );
							p( c, d ) >= 0 &&
				...
				( a.fn.select2 = function () {
					let d;
						...
						const j = [ 'opened', 'isFocused', 'container', 'dropdown' ];
						const k = [ 'val', 'data' ];
						const l = { search: 'externalSearch' };
					return (
						this.each( function () {
				...
				( a.fn.select2.defaults = {
					...
					formatSelection ( a, c, d ) {
						return a ? d( this.text( a ) ) : b;
					},
					sortResults ( a ) {
						return a;
					},
					formatResultCssClass ( a ) {
						return a.css;
					},
