...
class Customizer {
	...
	public static function init() {
		...
		if ( is_customize_preview() ) {
			add_action( 'customize_register', array( self::class, 'removeSections' ) );
			add_action( 'customize_register', array( self::class, 'addSections' ) );
		}
	}
			...
			array(
				...
				'fields' => array(
					...
					array(
						...
						'default' => '',
						'choices' => array_merge( array( '' => esc_html__( 'System font', 'chipmunk' ) ), self::getGoogleFonts() ),
					),
					array(
						'name'    => 'heading_font',
						'type'    => 'select',
	...
	private static function getGoogleFonts() {
		...
		if ( false === ( $googleFonts = unserialize( get_transient( THEME_SLUG . '_google_fonts' ) ) ) ) {
			$googleFonts = Helpers::getGoogleFonts( 'AIzaSyBF71G0SfVTAJVZGC5dilfzC1PunP0qAtE' ) ?? array();
	...
	private static function registerField( $customize, $section, $field ) {
		...
		$controlArgs = array(
			...
			'choices'     => array_filter( $field['choices'] ?? array() ),
			'input_attrs' => array_filter( $field['input_attrs'] ?? array() ),
		);
