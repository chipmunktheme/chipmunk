...
class EDD_Theme_Updater_Admin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		// Strings passed in from the updater config
		$this->menu_url = admin_url( 'admin.php?page=' . $this->theme_slug );
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = __( 'An error occurred, please try again.', 'chipmunk' );
			}

			$redirect = add_query_arg( array( 'sl_theme_activation' => 'false', 'message' => urlencode( $message ) ), $this->menu_url );
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							__( 'Your license key expired on %s.', 'chipmunk' ),
					...
					case 'revoked' :
						...
						$message = __( 'Your license key has been disabled.', 'chipmunk' );
						break;

					case 'missing' :

						$message = __( 'Invalid license.', 'chipmunk' );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = __( 'Your license is not active for this URL.', 'chipmunk' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'chipmunk' ), $this->item_name );
						break;

					case 'no_activations_left':

						$message = __( 'Your license key has reached its activation limit.', 'chipmunk' );
						break;
 ...
 */
function chipmunk_updater_admin_notices( $errors ) {
	if ( isset( $_GET['sl_theme_activation'] ) && ! empty( $_GET['message'] ) ) {
		if ( $_GET['sl_theme_activation'] == 'false') {
...
add_filter( 'chipmunk_admin_notices', 'chipmunk_updater_admin_notices' );
